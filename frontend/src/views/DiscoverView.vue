...
function scrollSection(container: HTMLElement | null, direction: 'left' | 'right') {
  ...
  if (direction === 'right') {
    // If we've reached or nearly reached the end, loop back to start
    ...
    // If we're at the very beginning, jump to the end
    if (currentScroll <= 10) {
...
<style scoped>
/* Low-Profile Scrollbar Track */
.custom-shelf-scroll::-webkit-scrollbar {
  height: 5px;
}
.custom-shelf-scroll::-webkit-scrollbar-track {
  background: rgba(30, 27, 24, 0.05);
  border-radius: 4px;
}
.custom-shelf-scroll::-webkit-scrollbar-thumb {
  background: rgba(30, 27, 24, 0.25);
  border-radius: 4px;
}
.custom-shelf-scroll::-webkit-scrollbar-thumb:hover {
  background: #C1432E;
}
